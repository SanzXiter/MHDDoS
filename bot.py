...
def handle_start(message):
    ...
    if result:
        ...
        if datetime.now() > expiration_date:
            vip_status = "❌ *VIP Kamu telah kedaluwarsa.*"
        else:
            dias_restantes = (expiration_date - datetime.now()).days
            vip_status = (
                f"✅ PELANGGAN VIP!\n"
                f"⏳ Hari Tersisa: {dias_restantes} dia(s)\n"
                f"📅 Kadaluarsa Pada: {expiration_date.strftime('%d/%m/%Y %H:%M:%S')}"
            )
    else:
        vip_status = "❌ *Kamu tidak memiliki VIP, Hubungi Owner: @blackkrizz .*"
...
def handle_addvip(message):
    if message.from_user.id != ADMIN_ID:
        bot.reply_to(message, "❌ Kamu bukan penjual real.")
        return
...
def handle_ping(message):
    ...
    if not result:
        bot.reply_to(message, "❌ Kamu tidak memiliki izin untuk menggunakan perintah ini.")
        return
    ...
    if telegram_id in cooldowns and time.time() - cooldowns[telegram_id] < 10:
        bot.reply_to(message, "❌ Tunggu 10 detik sebelum memulai ddos lain dan ingat untuk menghentikan ddos sebelumnya.")
        return
...
def handle_stop_attack(call):
    ...
    if call.from_user.id != telegram_id:
        bot.answer_callback_query(
            call.id, "❌ Hanya pengguna yang memulai ddos yang dapat menghentikannya."
        )
        return
    ...
    if telegram_id in active_attacks:
        ...
        bot.answer_callback_query(call.id, "✅ ddos berhasil dihentikan .")
        ...
        bot.answer_callback_query(call.id, "❌ Jika Anda tidak menemukan ddos, lanjutkan tindakan Anda.")
